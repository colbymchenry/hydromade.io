            ...
            return (
                <div key={theme.id} className={`d-flex justify-content-between w-100 align-items-center ${styles.theme}`}>
                    ...
                    <div className={'d-flex'}>
                        <ButtonSubmit type={"button"} hideArrow={isCurrentTheme} label={isCurrentTheme ? "edit" : "deploy"} processingLabel={"deploying"} processing={deployingTheme && deployingTheme.id === theme.id} onClick={() => isCurrentTheme ? router.push("/app/store/edit") : deployTheme(theme)} className={`${isCurrentTheme ? styles.blueBtn : 'btn-black'} ${styles.deploy}`} style={{ marginRight: '24px' }} />
