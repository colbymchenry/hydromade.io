...
export default function CreateAccountModal(props) {
    ...
    return (
        <animated.div style={close ? GoOutOfView(800, () => props.close()) : ComeIntoView()} className={styles.container}>
            <button type="button" onClick={() => setClose(true)}><FontAwesomeIcon icon={faX} /></button>
            <div>
                <animated.div style={close ? FadeOutDown(500) : FadeInUp(200)} className={`w-100 ${styles.margin1}`}>
                    <input type="email" placeholder="me@example.com" />
                    <div className={styles.label}><small>Email</small></div>
                </animated.div>
                <animated.div style={close ? FadeOutDown(400) : FadeInUp(300)} className={`w-100 ${styles.margin2}`}>
                    <input type="password" placeholder="&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;" />
                    <div className={styles.label}><small>Password (at least 8 characters)</small></div>
                </animated.div>
                <animated.div style={close ? FadeOutDown(300) : FadeInUp(400)} className={`w-100 ${styles.margin3}`}>
                    <input type="password" placeholder="&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;" />
                    <div className={styles.label}><small>Password Confirmation</small></div>
                </animated.div>

                <animated.div style={close ? FadeOutDown(200) : FadeInUp(500)} className={`w-100 d-flex justify-content-center ${styles.margin4}`}>
