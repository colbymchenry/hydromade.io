...
export default function CreateAccountModal(props) {
    ...
    return (
        <animated.div style={close ? GoOutOfView(2000, () => props.close()) : ComeIntoView()} className={styles.container}>
            <button type="button" onClick={() => setClose(true)}><FontAwesomeIcon icon={faX} /></button>
            <div>
                <animated.div style={close ? FadeOutDown(1800) : FadeInUp(1000)} className={`w-100 ${styles.margin1}`}>
                    <input type="email" placeholder="me@example.com" />
                    <div className={styles.label}><small>Email</small></div>
                </animated.div>
                <animated.div style={close ? FadeOutDown(1500) : FadeInUp(1200)} className={`w-100 ${styles.margin2}`}>
                    <input type="password" placeholder="&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;" />
                    <div className={styles.label}><small>Password (at least 8 characters)</small></div>
                </animated.div>
                <animated.div style={close ? FadeOutDown(1200) : FadeInUp(1500)} className={`w-100 ${styles.margin3}`}>
                    <input type="password" placeholder="&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;" />
                    <div className={styles.label}><small>Password Confirmation</small></div>
                </animated.div>

                <animated.div style={close ? FadeOutDown(1000) : FadeInUp(1800)} className={`w-100 d-flex justify-content-center ${styles.margin4}`}>
