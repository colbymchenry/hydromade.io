...
export const getUserByUID = async (uid) => {
    let user = await runAdminQuery(await admin.firestore().collection("users").where("uid", "==", uid).get());
    user = user.length ? user[0] : undefined;
    return user;
}

export const createUserWithUID = async (uid) => {
    await admin.firestore().collection("users").add({ uid });
    let user = await runAdminQuery(await admin.firestore().collection("users").where("uid", "==", uid).get());
    user = user[0];
    return user;
}
